...
const Signup = () => {
    // 1. URL se token nikalne ke liye hook
    ...
    const handleSignup = async (e) => {
        ...
        try {
            // 2. Payload mein token add karna
            const payload = {
                ...formData,
                invite_token: inviteToken // Agar null hoga toh backend ignore kar dega
            };
                        ...
                        {!inviteToken ? (
                            <div className="space-y-3 pt-1">
                                ...
                                <RadioGroup value={formData.role} onValueChange={(val) => setFormData({...formData, role: val})} disabled={loading} className="flex space-x-4">
                                    ...
                                    <div className="flex items-center space-x-2">
                                        <RadioGroupItem value="caretaker" id="caretaker" /> {/* Fixed value to 'caretaker' */}
                                        <Label htmlFor="caretaker" className="cursor-pointer">Care Taker</Label>
                                    </div>
                                </RadioGroup>
                            </div>
                        ) : (
                            <div className="pt-2">
                                <div className="p-3 bg-blue-50 border border-blue-200 rounded-md text-sm text-blue-700 text-center font-medium">
                                    ✓ Doctor invitation linked successfully.
                                </div>
                            </div>
                        )}
